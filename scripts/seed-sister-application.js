// @ts-nocheck
// scripts/seed-sister-application.js
// Run this with: node scripts/seed-sister-application.js <email>
// Creates a sister auth user + approved application, but no profile (simulates post-approval, pre-onboarding state)
...
if (!email) {
  console.error('Usage: node scripts/seed-sister-application.js <email>');
  process.exit(1);
}
...
async function seedSisterApplication() {
  console.log(`\nSeeding sister application for: ${email}\n`);
  ...
  // Create approved sister application (no profile)

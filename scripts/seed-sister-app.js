// @ts-nocheck
// scripts/seed-sister-app.js
// Usage: node scripts/seed-sister-app.js <email>
// Creates an approved sister application so you can test the onboarding flow.
...
if (!email) {
  console.error('Usage: node scripts/seed-sister-app.js <email>');
  process.exit(1);
}
...
async function deleteExistingUser(userId) {
  ...
  const profileId = sisterProfile?.id || brotherProfile?.id;
  const profileType = sisterProfile ? 'sister' : 'brother';
...
async function run() {
  console.log(`\nCreating approved sister application for: ${email}\n`);
  ...
  // Create approved sister application
  const { error: appError } = await supabase.from('sister_application').insert({
    user_id: userId,
    first_name: 'Aisha',
    last_name: 'Test',
    email,
